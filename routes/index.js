...
module.exports = function (app, addon) {
  ...
  app.post('/webhook',
    addon.authenticate(),
    function (req, res) {
      var textArray = req.body.item.message.message.split(" ");
      var zip;
      var re = /^\d{5,6}$/;
      ...
      geocoder.geocode(zip, function(err, res) {
        if (res === undefined){
          hipchat.sendMessage(req.clientInfo, req.identity.roomId, "Can't find anything for this zip code.")
          .then(function (data) {
            res.sendStatus(200);
          });
        } else {
          ...
          forecast.get([latitude, longitude], function(err, weather) {
            ...
            var address = resObject.formattedAddress;
            var city = resObject.city !== undefined ? resObject.city : zip;

            var message = 'You are requesting the weather in ' + zip + ' (' + address + ')' +
            "<br>" +
            'In ' + city + ' the forecast is: ' + summary;
